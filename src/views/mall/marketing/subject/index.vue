<template>
  <div>
    ...
    <div v-show="!showEdit">
      <el-table :data="tableData" row-key="subjectId" border stripe>
        <el-table-column type="index" label="序号" width="60">
        </el-table-column>
        <el-table-column prop="image" label="专题图片" width="200">
      ...
      <el-tabs type="border-card">
        <el-tab-pane label="基本信息">
          ...
          >
            ...
            <el-row>
              <el-col :span="5">
                <el-form-item label="描述" prop="subjectDescription">
                  <el-input v-model="form.subjectDescription"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
let form = ref({ status: 1, image: "", subjectItemSaleList: [] });
let tableData = ref([]);
let rules = ref({
  image: [{ required: true, message: "请选择专题图片", trigger: "blur" }],
...
function handleUpdate(row) {
  selectSubjectById(row.subjectId)
    .then((res) => {
      form.value = res.data;
      showEdit.value = true;
    })
...
function submitForm() {
  form.value.subjectItemSaleList = form.value.subjectItemSaleList.filter(
    (line) => line.itemCode
  );
  formValidate.value.validate((valid) => {
    if (valid) {
      if (form.value.subjectId != null) {
        updateSubject(form.value)
          .then((res) => {
            showEdit.value = false;
            resetForm();
...
function addTableLine() {
  form.value.subjectItemSaleList.push({});
}
function handleDeleteLine(index) {
  form.value.subjectItemSaleList.splice(index, 1);
}
function handleClickItemSale(selectionItems) {
  itemSaleTable.value.dialogTableVisible = false;
  selectionItems.forEach((item) => {
    var filterItem = form.value.subjectItemSaleList.filter(
      (element) => element.itemCode === item.itemCode
    );
    if (filterItem.length == 0) {
      form.value.subjectItemSaleList.splice(
        form.value.subjectItemSaleList.length - 1,
        0,
        item
      );
    }
