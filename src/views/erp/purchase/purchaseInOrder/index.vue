        ...
        <el-tab-pane label="基本信息">
          <el-form ref="formValidate" :model="form" label-width="80px">
            ...
            <el-row>
              <el-col>
                <el-table
                  :data="form.purchaseInOrderLineList"
                  row-key="skuCode"
                  style="width: 100%"
                >
                  <el-table-column type="index" label="序号" width="60">
...
<script setup name="PurchaseInOrder">
import {
  pagePurchaseInOrder,
  selectPurchaseInOrderById,
  addPurchaseInOrder,
  updatePurchaseInOrder,
  deletePurchaseInOrder,
  auditPurchaseInOrder,
} from "@/api/purchaseInOrder";
...
let form = ref({ purchaseInOrderLineList: [] });
let tableData = ref([]);
...
function addTableLine() {
  form.value.purchaseInOrderLineList.push({ quantity: 0, price: 0, amount: 0 });
}
...
function loadData() {
  pagePurchaseInOrder(query.value)
    .then((res) => {
...
function resetForm() {
  form.value = { purchaseInOrderLineList: [] };
}
function backTable() {
  resetForm();
...
function handleUpdate(row) {
  selectPurchaseInOrderById(row.purchaseInOrderId)
    .then((res) => {
      showEdit.value = true;
      isEdit.value = true;
...
function handleDelete(row) {
  ElMessageBox.confirm("此操作将永久删除该记录, 是否继续?", "提示", {
    ...
    .then(() => {
      deletePurchaseInOrder(row.purchaseInOrderId)
        .then((res) => {
...
function handleDeleteLine(index) {
  form.value.purchaseInOrderLineList.splice(index, 1);
}
...
function handleAudit(row) {
  ElMessageBox.confirm("是否确认审核？", "提示", {
    ...
    .then(() => {
      auditPurchaseInOrder(row.purchaseInOrderId)
        .then((res) => {
...
function submitForm() {
  formValidate.value.validate((valid) => {
    if (valid) {
      //去除空行
      form.value.purchaseInOrderLineList =
        form.value.purchaseInOrderLineList.filter((line) => line.skuCode);
      if (form.value.purchaseInOrderId != null) {
        updatePurchaseInOrder(form.value)
          .then((res) => {
            showEdit.value = false;
            resetForm();
...
function handleClickItemSaleSku(selectionItems) {
  ...
  selectionItems.forEach((item) => {
    let filterItem = form.value.purchaseInOrderLineList.filter(
    ...
    if (filterItem.length == 0) {
      ...
      item.price = 0;
      item.amount = 0;
      form.value.purchaseInOrderLineList.splice(
        form.value.purchaseInOrderLineList.length - 1,
        0,
        item
      );
    }
